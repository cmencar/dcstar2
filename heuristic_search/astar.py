...
class AStar:
    ...
    def __get_heuristic_value(self, node):
        return self.__get_first_level_heuristic_value(node) + self.__get_second_level_heuristic_value_wsc(node)
    ...
    # Method for acquiring the second-level heuristic value. It is based on the similarity between the individual
    # generated by AStar and the one generated by DGG. This one in particular could use the internal method
    # "worst_case_scenario", forcing DGG to generate a pure individual, even one with all possible cuts activated
    # @node: node of to be evaluated to get the second-level heuristic
