...
class DCStarProblem(Problem):
    ...
    def estimate_cost(self, node):
        ...
        # calculating the g+h cost
        g_plus_h = self.g(node) + h

        # calculating the gg cost
        third_level_priority = self.__get_second_level_heuristic_value(node)

        return g_plus_h, h, third_level_priority
    ...
    # TODO TRADURRE I COMMENTI DA ITALIANO AD INGLESE
    # Method for acquiring the heuristic value. It is based on the maximum number of cuts to be added for every group
    # of connected hyperboxes
    ...
    def __get_cuts_number_to_add(self, hyperbox):
        ...
        # if the number of cuts to be added is less than the number of different classes in the hyperbox,
    ...
    def __second_level_priority(self, node):
    ...
    def __third_level_priority(self, node):
